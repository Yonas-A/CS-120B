...
int main(void) {
    ...
    unsigned char icon = 0x00; // a temporary var. to indicate PA[4:0] < 5
    unsigned char level = 0x00; // a temporary var. to hold fuel level
    unsigned char ignition = 0x00; // a temporary var. to hold PA4
    unsigned char seated = 0x00; // a temporary var. to hold PA5
    unsigned char belt = 0x00; // a temporary var. to hold PA6
    unsigned char beltWarning = 0x00; // a temporary var. to hold fuel level
    
    while (1) {

        // initialize values
        PORTC = 0x00; icon = 0x00; level = 0x00;  beltWarning = 0x00; 
        
        tmpA = PINA & 0x0F;             // set tmpA to A[3:0]
        ignition = (PINA >> 4) & 0x01;  // set tmpA to A4 
        seated = (PINA >> 5) & 0x01;    // set tmpA to A5
        belt = (PINA >> 6) & 0x01;      // set tmpA to A6
        ...
        // if ignition and seated are 1 but belt is 0, PC7 = 1xxxxxxx, 
        //                                  so set seat warning to 0x80
        if(belt == 0x00 && ignition == 0x01 && seated == 0x01)   beltWarning = 0x80;

        PORTC = ((level | icon) | beltWarning);
    }
    return 1;
}