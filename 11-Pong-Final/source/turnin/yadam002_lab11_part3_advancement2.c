 ...
 *      Pong game with a a second player mode added along with a simple menu that
 *               will select between player v Ai and player v player mode.
 * 
 *      Note: this advancement is made on top of the first advancement. So the
 *          demo video holds both demos in a same video as it was easier to 
 *          demonstrate the ball spin when both sets of paddles are controlled
 *          by AI as we can see predict the direction and explain how it happened.
 * 
...
static unsigned char ballSpeed = 100;  // holds the period for the pong ball
...
int score_tick(int state) {
    ...
    PORTB = (is2Player) ? output | 0x80 : output;
    return state;
}
...
int main(void) {
    ...
    task7.state = start;
    task7.period = 200;
    task7.elapsedTime = task4.period;
    task7.TickFct = &paddle1_tick;

    // Task 8 Tick_menu
    task8.state = start;
    task8.period = 200;
    task8.elapsedTime = task4.period;
    task8.TickFct = &Tick_menu;
